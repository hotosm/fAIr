...
def verify_schema():
    ...
    print("\nVerifying ApiTransactionSchema variants...")
    ...
    print("Checking DDL for result column in ApiTransactionSchema (Postgres)...")
    if "result TEXT" in sql_str_api or "result\" TEXT" in sql_str_api:
         print("SUCCESS: result column is TEXT in Postgres DDL.")
    else:
         print("FAILURE: result column is NOT TEXT in Postgres DDL. DDL:")
         print(sql_str_api)
         sys.exit(1)

    print("\nVerifying Identifier Lengths...")
    from zenml.zen_stores.schemas.schema_utils import get_index_name, foreign_key_constraint_name
    
    # Test Index Name
    # Create a long index name input
    long_table = "some_very_long_table_name_that_takes_up_space"
    long_cols = ["some_column_that_is_also_very_long", "another_column_that_is_long", "and_a_third_one"]
    ...
    fk_name = foreign_key_constraint_name(long_table, "target_table_is_also_long", "source_col_is_long")
