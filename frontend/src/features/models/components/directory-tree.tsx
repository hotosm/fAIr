...
import { SlTree, SlTreeItem } from '@shoelace-style/shoelace/dist/react/index.js';
...
const FileItem = ({ keyName, size, onDownload, isDownloading }: { keyName: string, size: number, onDownload: () => void, isDownloading: boolean }) => (
    <div className='flex items-center gap-x-2' onClick={onDownload}>
        <FileIcon className="w-4 h-4" />
        <div className='flex flex-col md:flex-row gap-x-2'>
            <span title={keyName} className='text-dark text-nowrap text-body-2base'>{truncateString(keyName)}</span>
...
const DirectoryItem = ({ keyName, size, length, children }: { keyName: string, size: number, length: number, children: React.ReactNode }) => (
    <>
        <div className='flex items-center gap-x-2'>
            ...
            <div className='flex flex-col md:flex-row gap-x-2'>
                ...
                <div className='flex gap-x-2'>
                    <span className='text-gray text-body-3 text-nowrap'><SlFormatBytes value={size} /></span>
                    <span className='text-gray text-body-3 text-nowrap'>{length} items</span>
                </div>
            </div>
        </div>
        {children}
...
const DirectoryTree: React.FC<DirectoryTreeProps> = ({ datasetId, trainingId }) => {
    ...
    const fetchDirectoryRecursive = async (currentDirectory: string = ''): Promise<any> => {
    ...
    const handleFileDownload = async (validPath: string) => {
        try {
            ...
            const response = await apiClient.get(API_ENDPOINTS.DOWNLOAD_TRAINING_FILE(datasetId, trainingId, validPath), {
                responseType: 'blob',
            });
            ...
            //@ts-expect-error
            const errorMessage = error.response?.statusText || 'Failed to download file.';
