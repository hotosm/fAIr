...
const AccuracyDisplay = ({ accuracy }: { accuracy: number }) => {
  const colors = ["bg-[#F33A0C]", "bg-[#FF7802]", "bg-[#FFCF16]", "bg-[#86CE02]", "bg-[#00BF11]"];

  const thresholds = [50, 60, 70, 80, 100];

  const activeIndex = thresholds.findIndex((threshold) => accuracy < threshold);
  const activeColor = colors[activeIndex === -1 ? colors.length - 1 : activeIndex];

  const arrowPosition = (accuracy / 100) * 100;

  return (
    <div className="flex items-center gap-x-4">
      <span className="text-dark font-semibold text-title-3">{roundNumber(accuracy)}</span>
