...
const ModelCard: React.FC<ModelCardProps> = ({ model }) => {
    ...
    return (
        <div className="flex items-center  w-full">
            <Link nativeAnchor={false} disableLinkStyle href={`${APPLICATION_ROUTES.MODELS}/${model.id}`} title={model.name} className="w-[300px] mx-auto min-h-[450px] flex flex-col border border-gray-border hover:shadow-md overflow-hidden group">
                ...
                <div className="p-5 flex flex-col gap-y-6">
                    <div className="inline-flex flex-col gap-y-2">
                        <p className="font-medium text-body-1 text-black">{truncateString(model.name, 20)}</p>
                        <p className="text-gray text-body-2">ID: <span>{model.id}</span></p>
                    </div>
                    {/* accuracy */}
                    <div>
                        <p className="text-gray text-body-3">Accuracy:</p>
                        <p className="text-dark font-semibold text-body-2">{roundNumber(model.accuracy)} %</p>
                    </div>
                    {/* Name and date */}
                    <div className="inline-flex flex-col gap-y-2">
