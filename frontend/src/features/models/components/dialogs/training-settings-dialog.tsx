...
import { MODEL_CREATION_FORM_NAME, ModelsProvider, useModelsContext } from "@/app/providers/models-provider";
...
const ModelTrainingSettingsDialog: React.FC<ModelEnhancementDialogProps> = ({
  ...
  return (
    <ModelsProvider>
      ...
      <UpdateBaseModel handleChange={handleChange} baseModel={data?.base_model as string} />
      <Dialog
        isOpened={isOpened}
...
const UpdateBaseModel = ({ baseModel, handleChange }: { baseModel: string, handleChange: (k: string, v: string) => void }) => {
  useEffect(() => {
