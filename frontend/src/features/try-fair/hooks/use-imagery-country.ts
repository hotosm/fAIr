...
import { CountryResult, reverseGeocodeCountry } from "@/features/try-fair/api/hot-imagery";
 ...
 */
export const useImageryCountry = (bounds: BBOX | null | undefined): CountryResult | null => {
  const center = bounds ? [(bounds[0] + bounds[2]) / 2, (bounds[1] + bounds[3]) / 2] : null;

  const { data } = useQuery({
    queryKey: ["imagery-country", center?.[0], center?.[1]],
    queryFn: ({ signal }) => reverseGeocodeCountry(center![0], center![1], signal),
    enabled: !!center,
    staleTime: Infinity,
    gcTime: Infinity,
  });
