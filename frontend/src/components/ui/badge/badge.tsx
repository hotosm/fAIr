...
const Badge: React.FC<BadgeProps> = ({ variant, children, rounded = false, onClick, className }) => {
    ...
    return (
        <button className={`h-8 w-fit ${variants[variant]} ${rounded ? 'rounded-full p-2' : 'rounded-2xl py-1 px-3 '} ${className}`} onClick={onClick}>
