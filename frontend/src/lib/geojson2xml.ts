...
function geojson2osm(geojson: FeatureCollection): string {
  ...
  const doc = create({ declaration: false })
    .ele('osm', { version: '0.6', generator: 'geojson2osm' });

  let lastNodeId = -1;
  nodes.forEach((node) => {
    const nodeEl = doc.ele('node', { id: lastNodeId--, lat: node.lat, lon: node.lon });
  ...
  relations.forEach((relation) => {
    ...
    relation.members.forEach((member) => {
      relationEl.ele('member', { type: member.type, ref: member.elem.id, role: member.role });
    });
...
function processPolygon(
  ...
  coordinates.forEach((polygon, index) => {
    const way = new Way({});
    ways.push(way);
    relation.members.push({ elem: way, type: 'way', role: index === 0 ? 'outer' : 'inner' });
