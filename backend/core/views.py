...
class DatasetApiView(APIView):
    def post(self, request,dataset_id,source=None, *args, **kwargs):
        ...
        return Response("Download Success", status=status.HTTP_201_CREATED)
...
def download_imagery(start : list , end :list , zm_level , dataset_id , source='maxar'):
    ...
    while start_x <= stop_x: # download for x section while keeping y as constant 
        ...
        with open(f"{base_path}/{source}-{start_x}-{begin_y}-{zm_level}.png", 'wb') as handle:
